...
std::unique_ptr<lst::LSTESData<alpaka_common::DevHost>> lst::loadAndFillESHost() {
  ...
  EndcapGeometryBuffer<alpaka_common::DevHost> endcapGeometryBuffers(
  ...
  lst::loadModulesFromFile(pLStoLayer,
                           path.c_str(),
                           nModules,
                           nLowerModules,
                           nPixels,
  ...
  return std::make_unique<LSTESData<alpaka_common::DevHost>>(
      nModules, nLowerModules, nPixels, endcapGeometry.nEndCapMap, modulesBuffers, std::move(endcapGeometryBuffers), pixelMappingPtr);
}
